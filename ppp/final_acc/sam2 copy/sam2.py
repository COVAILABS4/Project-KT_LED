...
KIT_NO = Local.get("KIT_NO", KIT_NO)
STATIC_IP = Local.get("STATIC_NO", STATIC_IP)

if STATIC_IP=="":
    STATIC_IP = KIT_NO


SERVER_NO = Local.get("SERVER_NO", SERVER_NO)
SSID = Local.get("SSID", SSID)
PASSWORD = Local.get("PASSWORD", PASSWORD)
...
def connect_to_wifi(ssid, password):
    ...
    global server_ip , sta,STATIC_IP,SERVER_NO;
    start_sta();
...
# except Exception as err:
#     pass
    # free_memory_kb = gc.mem_free() / 1024  # Free RAM in KB
    # allocated_memory_kb = gc.mem_alloc() / 1024  # Allocated RAM in KB

    # # ROM (Flash Storage) information in KB
    # flash_size_kb = esp.flash_size() / 1024  # Flash size in KB

    # print(f"Free RAM: {free_memory_kb:.2f} KB")
    # print(f"Allocated RAM: {allocated_memory_kb:.2f} KB")
    # print(f"Total Flash (ROM) Size: {flash_size_kb:.2f} KB")
